...
@implementation WJValidationView
...
- (instancetype)initWithFrame:(CGRect)frame andCharCount:(NSInteger)charCount andLineCount:(NSInteger)lineCount {
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = kMainGaryWhiteColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    NSInteger charWidth = rectWidth / text.length - ScaleW(15);
    ...
    for (NSInteger i = 0; i < text.length; i++) {
        ...
        [textC drawAtPoint:CGPointMake(pointX, pointY) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:arc4random() % 10 + ScaleW(15)]}];
    }

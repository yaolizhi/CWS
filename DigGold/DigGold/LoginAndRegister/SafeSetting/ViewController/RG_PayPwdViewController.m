...
@interface RG_PayPwdViewController ()
...
@implementation RG_PayPwdViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    if ([[SSKJ_User_Tool sharedUserTool].getHas_tpwd isEqualToString:@"1"]) {
        self.title = Localized(@"修改支付密码", nil);
    }else{
        self.title = Localized(@"设置支付密码", nil);
    }
...
- (void)resetPayButtonFetchData {
    ...
    NSDictionary *params = @{@"mobile":self.phoneTF.text?:@"",
                             ...
                             @"tpwd1":self.pwdAgainTF.text?:@"",
                             @"account":[[SSKJ_User_Tool sharedUserTool]getAccount]?:@""};
    
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:Port_ResetTPWD_URL RequestType:RequestTypePost Parameters:params Success:^(NSInteger statusCode, id responseObject) {
...
- (void)setupMasnory {
    ...
    [self.phoneTF mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.contentView).offset(ScaleH(24));
    ...
    [self.pwdTF mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.contentView).offset(ScaleW(14));
        make.right.equalTo(self.contentView).offset(-ScaleW(14));
        make.top.equalTo(self.moreFunView.mas_bottom).offset(ScaleH(10));
        make.height.mas_equalTo(ScaleH(45));
...
- (UITextField *)phoneTF {
    if (!_phoneTF) {
        ...
        _phoneTF.text = [[SSKJ_User_Tool sharedUserTool]getMobile]?:@"";
...
- (UITextField *)pwdAgainTF {
    if (!_pwdAgainTF) {
        ...
        _pwdAgainTF.placeholder = Localized(@"重复密码", nil);

...
- (void)openConnectSocketWithConnectSuccess:(SocketDidConnectBlock)connectSuccess {
    [ManagerSocket sharedManager].connect = connectSuccess;
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    [ManagerSocket sharedManager].connect ? [ManagerSocket sharedManager].connect() : nil;
    // 开启成功后重置重连计数器
    _reconnectCounter = 0;
    [self clearReconnectSocketTimer];
   // 开启发送定时器数据
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message{
    [_delegate socketDidReciveData:message];

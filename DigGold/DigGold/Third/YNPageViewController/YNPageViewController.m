...
- (void)setupBottomView {
    ...
    CGFloat bottomHeight = self.config.showBottomView ? self.config.bottomViewHeight : 0;
    CGFloat contentTop = kYNPAGE_SCREEN_HEIGHT - navHeight - tabHeight - bottomHeight;
    if (self.config.showBottomView && self.bottomView) {
        self.bottomBgView = [[UIView alloc]initWithFrame:CGRectMake(0, contentTop, ScreenWidth, self.config.bottomViewHeight)];
...
- (void)setupPageScrollView {
    ...
    CGFloat contentHeight = kYNPAGE_SCREEN_HEIGHT - navHeight - tabHeight - bottomHeight;
    if ([self isSuspensionTopPauseStyle]) {

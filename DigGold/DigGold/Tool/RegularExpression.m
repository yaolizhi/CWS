...
+ (BOOL)validateProtectionWith:(NSString *)protection {
    NSString *regex= @"^1[45][0-9]{7}|([P|p|S|s]\\d{7})|([S|s|G|g]\\d{8})|([Gg|Tt|Ss|Ll|Qq|Dd|Aa|Ff]\\d{8})|([H|h|M|m]\\d{8，10})$";
     NSPredicate *identityCardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    return [identityCardPredicate evaluateWithObject:protection];
}
...
+ (BOOL)isSeparatedPureInt:(NSString *)string {
    if (string.length == 0) {
        return YES;
    }
    NSArray *array = [string componentsSeparatedByString:@"."];
    if (array.count > 0) {
        NSString *backS = array[1];
        if ([backS integerValue] > 0) {
            return NO;
        }else{
            return YES;
        }
